...
- `shapes` _Array&lt;String&gt;_: An array of shapes for the confetti. The possible values are `square`, `circle` and `image`. The default is to use both shapes in an even mix. You can even change the mix by providing a value such as `['image', 'circle', 'square']` to use two third circles and one third squares.
...
- `imageSrc` _String (default: '') : Image path.
